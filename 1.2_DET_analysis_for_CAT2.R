###############################################################################
###############################################################################
# Title: Differential Expression Analysis for CAT2: Ischemic Stroke
# Author: Rashi Verma
# Date: 2025-11-13
# Description: 
...
dim(targets)  
filtered_targets <- targets %>% filter(treatment %in% c("TPA", "TPA+MT") | final_diagnosis == "Mimic")
filtered_targets_clean <- filtered_targets %>% filter(!is.na(age) & !is.na(severity_score))
head(filtered_targets)
dim(filtered_targets_clean)

countsTable <- read.csv(file='4_transcript_count_matrix_batch1.csv')
dim(countsTable)
...
exp_counts <- filtered_gene_counts[(rowSums(filtered_gene_counts) > 67),]
dim(exp_counts)

filtered_targets_clean$final_diagnosis_binary <- ifelse(filtered_targets_clean$final_diagnosis == "Stroke", 1, 0)
dim(filtered_targets_clean)
...
# Voom
v <- voom(dge, design, weights = final_targets$weights, normalize = "quantile", plot = TRUE)

# Linear modeling
fit <- lmFit(v, design)
contrast.matrix <- makeContrasts(ISvsHS = Stroke - Mimic,levels = colnames(design))
fit <-lmFit(v, design)
...
# DE table
top.table <- topTable(efit, sort.by = "P", adjust.method = "BH", n = Inf)
filtered_genes <- top.table[top.table$P.Value < 0.05 & abs(top.table$logFC) > log2(1.5), ]
#write.csv(filtered_genes, "results/2_TPA_vs_SM_all_t.csv", row.names = TRUE)
...
#write.csv(sorted_genes, "results/2_TPA_vs_SM_50_t.csv", row.names = TRUE)
...
group_vector <- ifelse(final_targets$final_diagnosis == "Stroke", "Stroke", "Mimic")
...
ann_colors = list(Groups = c("Stroke" = "#0072B2", "Mimic" = "olivedrab"))
...
#tiff(file="plots/2_heatmap_CAT2.tiff", unit= "in", res = 600, width = 2.6, height = 2.5)
# Create the heatmap
pheatmap(
...
shape_mapping <- c("Mimic" = 22, "Stroke" = 21)
# Add diagnosis information (replace 'sample_diagnosis' with your actual diagnosis vector)
pca_df$Diagnosis <- final_targets$final_diagnosis  # Vector with "Hemorrhage" or "Stroke" corresponding to samples

# Modify color mapping: Hemorrhage (red), Stroke (blue)
fill_mapping <- c("Mimic" = "olivedrab", "Stroke" = "#0072B2")

#tiff(file="plots/2_PCA_CAT2.tiff", unit= "in", res = 600, width = 2.8, height = 2.5)
...
ggplot(pca_df, aes(x = PC1, y = PC2, shape = Diagnosis, fill = Diagnosis)) +
  ...
  #ggtitle("Stroke vs. Stroke Mimic") +
...
ggsave("plots/2_volcano_CAT2.tiff", plot = volcano_plot, width = 2.4, height = 2.4, units = "in", dpi = 600)
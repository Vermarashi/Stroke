...
# Hemorrhagic stroke vs. Others (Ischemic Stroke + Stroke Mimic)
# Author: Rashi Verma
# Date: 2025-11-13
# Description: 
...
train_df$Cat1 <- as.numeric(pheno_train$final_diagnosis == "Hemorrhage")


val_df <- expr_val_corrected
val_df <- as.data.frame(val_df)
val_df <- val_df[rownames(val_df) %in% rownames(pheno_val), ]
val_df$Cat1 <- as.numeric(pheno_val$final_diagnosis == "Hemorrhage")
...
# --------------- This loop will take day(s) to produce results ----------------
# ==============================================================================
...
perfect_genes <- c(perfect_genes$Gene)
perfect_df <- data.frame(GeneCombo = character(),
...
best_k <- 2  # your chosen number of hidden states

# Loop over combination sizes 2 to length of perfect_genes
for (n in 2:length(perfect_genes)) {
  gene_combos <- combn(perfect_genes, n, simplify = FALSE)
  ...
  for (glist in gene_combos) {
    ...
    cm_train <- confusionMatrix(factor(train_pred, levels=c(0,1)),
    ...
    cm_val <- confusionMatrix(factor(val_pred, levels=c(0,1)),
...
# Save results
write.csv(perfect_df, "plots/panel_CAT1.csv", row.names = FALSE)
...
best_k <- 2
perfect_genes <- c("ENST00000587014", "MSTRG_24342_56", "MSTRG_42624_2") # Three-transcript panel
perfect_genes <- c("ENST00000587014", "MSTRG_32734_241", "ENST00000465590", "MSTRG_50567_5") # Four-transcript panel
...
## Plot Fig.4D (Four transcript panel) and Fig.4E (three transcript panel)
...
tiff(file="plots/Fig_4D.tiff", unit= "in", res = 600, width = 2.307, height = 2.239)  # change to Fig_4E in case on three transcript panel
...
set.seed(1234)
## pheno_val$final_diagnosis contains "Stroke", "Mimic", "Hemorrhage"
val_plot_df$Cat1_detail <- with(val_plot_df, ifelse(actual == 1, "Hemorrhage",
                                                    ifelse(pheno_val$final_diagnosis == "Stroke", "Stroke", "Mimic")))
## Make it a factor for consistent coloring
val_plot_df$Cat1_detail <- factor(val_plot_df$Cat1_detail, levels = c("Stroke", "Mimic", "Hemorrhage"))

tiff(file="plots/Fig_4F.tiff", unit= "in", res = 600, width = 2.3, height = 2.3)
ggplot(val_plot_df, aes(x = actual, y = predicted, color = Cat1_detail)) +
  geom_jitter(width = 0.07, height = 0.07, size = 2, alpha = 0.8) +
  ...
  scale_color_manual(
    values = c("Stroke" = "#1f77b4", "Mimic" = "olivedrab", "Hemorrhage" = "darkorange"),
    name = "Stroke Type",
    labels = c("0" = "Stroke + Stroke Mimic", "1" = "Hemorrhage") 
    ) +
  theme_minimal() + 
  my_clean_theme +
  theme(
...
################################################################################################
## Supplementary Figure (Expression of top 4 gene)
## Plot Suppl_Fig
top4  <- glist_top40  

## Subset training data
expr_plot <- train_scaled[, top4, drop = FALSE]
expr_plot$Diagnosis <- factor(train_scaled$Cat1, labels = c("Other","Hemorrhage"))

## Melt for ggplot
expr_melt <- melt(expr_plot, id.vars = "Diagnosis", variable.name = "Gene", value.name = "Expression")

## Add a column to indicate top4 vs rest
expr_melt$Group <- ifelse(expr_melt$Gene %in% top4, "Top4", "Other5")

expr_summary <- expr_melt %>%
  group_by(Gene, Diagnosis, Group) %>%
  summarise(Mean = mean(Expression), SD = sd(Expression), .groups = "drop")

tiff(file="plots/Suppl_Fig_HMM_CAT1.tiff", unit= "in", res = 600, width = 2.3, height = 2.3)
ggplot(expr_summary, aes(x = Diagnosis, y = Mean, fill = Group)) +
  geom_bar(stat = "identity", position = position_dodge(width=0.8)) +
  geom_errorbar(aes(ymin = Mean-SD, ymax = Mean+SD), position = position_dodge(width=0.8), width = 0.2) +
  facet_wrap(~ Gene, scales = "fixed") +   # <- make y-axis same
  scale_fill_manual(values = c("Top4" = "red", "Other5" = "gray")) +
  theme_bw() +
  labs(title = "Mean Expression of Perfect Genes (Top 2 vs Other 5)")
dev.off()

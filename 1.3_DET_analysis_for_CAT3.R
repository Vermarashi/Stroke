###############################################################################
###############################################################################
# Title: Differential Expression Analysis for CAT3: Stroke Mimic
# Author: Rashi Verma
# Date: 2025-11-13
# Description: 
...
dim(targets)
filtered_targets <- targets %>% filter(final_diagnosis %in% c("Hemorrhage", "Mimic"))
...
exp_counts <- filtered_gene_counts[(rowSums(filtered_gene_counts) > 54),]
dim(exp_counts)
...
# Linear modeling
fit <- lmFit(v, design)
contrast.matrix <- makeContrasts(ISvsHS = Hemorrhage - Mimic,levels = colnames(design))
fit <-lmFit(v, design)
...
#write.csv(filtered_genes, "results/3_HS_vs_SM_all_t.csv", row.names = TRUE)
...
#write.csv(sorted_genes, "results/3_HS_vs_SM_50_t.csv", row.names = TRUE)
...
group_vector <- ifelse(final_targets$final_diagnosis == "Hemorrhage", "Hemorrhage", "Mimic")
group_df <- data.frame(Groups = group_vector)
rownames(group_df) <- colnames(sorted_gene_expression)

ann_colors = list(Groups = c("Mimic" = "olivedrab", "Hemorrhage" = "darkorange"))
...
#tiff(file="plots/3_heatmap_HS_vs_SM.tiff", unit= "in", res = 600, width = 2.6, height = 2.5)
# Create the heatmap
pheatmap(
  ...
  #main = "Hemorrhage vs. stroke mimic",  # Add title
...
shape_mapping <- c("Mimic" = 22, "Hemorrhage" = 24)
# Add diagnosis information (replace 'sample_diagnosis' with your actual diagnosis vector)
pca_df$Diagnosis <- final_targets$final_diagnosis  # Vector with "Hemorrhage" or "Stroke" corresponding to samples

# Modify color mapping: Hemorrhage (red), Stroke (blue)
fill_mapping <- c("Hemorrhage" = "darkorange", "Mimic" = "olivedrab")

#tiff(file="plots/3_PCA_HS_vs_SM.tiff", unit= "in", res = 600, width = 2.8, height = 2.5)
...
ggsave("plots/3_volcano_HS_vs_SM.tiff", plot = volcano_plot, width = 2.4, height = 2.4, units = "in", dpi = 600)
...
# Ischemic Stroke - Severity outcome
# Author: Rashi Verma
# Date: 2025-11-14
# Description: 
...
#   - CPM normalization
#   - Subset highly correlated transcripts to NIHSS ADMIT SCORE (continuous)
...
batch1_samples_clean <- subset(batch1_samples_clean, !(samples %in% c("S0085", "S0145", "S0227", "S0247", "S0580", "S0613"))) # HC conversion, second or multiple stroke
batch1_samples_clean$severity_score <- as.numeric(batch1_samples_clean$severity_score)
batch1_samples_clean <- subset(batch1_samples_clean, !(severity_score == 0))
...
batch2_samples <- subset(pheno_all, samples %in% c(batch2_names$sample_id, batch3_names$sample_id)) 
batch2_samples_clean <- subset(batch2_samples, !(severity_score == 0))
batch2_samples_clean <- subset(batch2_samples_clean, !(severity_score == ""))
batch2_samples_clean <- subset(batch2_samples_clean, clinical_diagnosis == "AIS")
...
pheno_val <- batch2_samples_clean[!batch2_samples_clean$samples %in% exclude_samples, ]
pheno_val$severity_score <- as.numeric(pheno_val$severity_score)
...
## Filter lowly expressed genes BEFORE CPM ----
...
# ==============================================================================
# ------------- Subset correlated transcripts to severity score ----------------
# ==============================================================================
...
# Optionally, reorder phenotype data to match
...
correlation_results <- apply(t(cpm_train), 1, function(x) {
  cor.test(x, pheno_train$severity_score, method = "pearson")
})
...
results[1:4,]
corgenes<-results[abs(results$correlation)>0.4 ,]  # 254
...
#write.csv(corgenes2,"traingenes_severity_score.csv")
...
mod_val <- model.matrix(~ severity_score, data = pheno_val) #Including final_diagnosis in the model preserves biological differences
...
threshold <- 4  # or dhours(3.5)
train_df$Cat1 <- as.numeric(pheno_train$severity_score > threshold)


val_df <- expr_val_corrected
val_df <- as.data.frame(val_df)
val_df <- val_df[rownames(val_df) %in% rownames(pheno_val), ]
val_df$Cat1 <- as.numeric(pheno_val$severity_score > threshold)
...
# Ensure no masking and lock caret version
rm(confusionMatrix)
...
## Save results
write.csv(perfect_df, "plots/panel_SS.csv", row.names = FALSE)
...
## whereas four genes are sufficient to classify ischemic patients outcome

best_k <- 2
perfect_genes <- c("MSTRG_10895_4", "MSTRG_11929_9", "ENST00000587709", "ENST00000694921")
...
tiff(file="plots/Fig_7A.tiff", unit= "in", res = 600, width = 2.3, height = 3)
p <- ggplot(top_genes, aes(x = reorder(genes, cor), y = cor, color = cor > 0)) +
  geom_point(size = 2) +
  coord_flip() +
  scale_color_manual(values = c("firebrick", "steelblue")) +
  labs(
    title = "Top 20 Genes Correlated with NIHSS Severity Score",
    x = "Transcripts",
...
dev.off()
print(p)
##ggsave("plots/Fig_7A.tiff", plot = p, width = 3.5, height = 5, dpi = 600)
...
tiff(file="plots/Fig_7B.tiff", unit= "in", res = 600, width = 2, height = 3)
##top_20_bp_pathways <- subset(top_20_go_pathways, GO_type == "BP")
top_20_go_pathways$Description_wrapped <- str_wrap(top_20_go_pathways$Description, width = 60)
...
##ggsave("plots/Fig_7B.tiff", plot = p, width = 3.748, height = 5.8, dpi = 600)
print(p)
##dev.off()
...
train_state_prob_df <- data.frame(
  ...
  NIHSS_Group = factor(
    train_scaled$Cat1,
    levels = c(0, 1),
    labels = c("NIHSS ≤ 4", "NIHSS > 4")
  )
)


p <- ggplot(train_state_prob_df,
            aes(x = NIHSS_Group, y = Prob, fill = NIHSS_Group)) +
  ...
  ## X-axis labels with sample size (adjust n if needed)
  scale_x_discrete(
    labels = c(
      "NIHSS ≤ 4\n(n = 12)",
      "NIHSS > 4\n(n = 26)"
...
ggsave("plot/Fig_7C.tiff", plot = p, width = 3.817, height = 2.621, dpi = 600)
...
## Plot ROC curves
tiff(file="plots/Fig_7D.tiff", unit= "in", res = 600, width = 2.307, height = 2.239)
...
tiff(file="plots/SeverityScore_HMM_CM.tiff", unit= "in", res = 600, width = 2.3, height = 2.3)
set.seed(1234)
